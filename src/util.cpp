...
const char * const BITCOIN_CONF_FILENAME = "xaitencoin.conf";
const char * const BITCOIN_PID_FILENAME = "xaitencoind.pid";
...
#else
    const char* pszModule = "xaitencoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XaitenCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/XaitenCoin";
#else
    // Unix
    return pathRet / ".xaitencoin";
#endif
#endif
}
